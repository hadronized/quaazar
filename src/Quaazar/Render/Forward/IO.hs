...
display :: (MonadIO m) => ForwardRenderer -> GPUFrame -> Viewport -> Post -> m GPUTexture
display (ForwardRenderer lighting shadowing accumulation window) gpuframe screenViewport pst = liftIO $ do
